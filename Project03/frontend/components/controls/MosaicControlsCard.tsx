...
export function MosaicControlsCard({ state, onChange, onRandomizeSeed }: MosaicControlsCardProps) {
    ...
    () => (
      [
        ...
        },
        {
          ...
          description: "Blend original patch into tiles for smoother transitions",
        },
        {
          id: "topK",
          label: "Top-K",
  ...
  return (
    <Card>
      <CardHeader className="pb-4">
        ...
        <CardDescription>Feature extraction, randomness, and fallback tuning.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6 pt-0">
        <div className="grid gap-5 md:grid-cols-2">
          <div className="space-y-3">
            <Label className="text-xs uppercase tracking-wide text-muted-foreground">Feature Method</Label>
            <Select value={state.featureMethod} onValueChange={(value: FeatureMethod) => onChange({ featureMethod: value })}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
          ...
          <div className="space-y-3">
            <Label className="text-xs uppercase tracking-wide text-muted-foreground">Seed</Label>
            <div className="flex items-center gap-3">
              <Input
                ...
                className="w-32"
              />
              <Button type="button" variant="ghost" size="sm" onClick={onRandomizeSeed}>
            ...
            <div key={slider.id} className="space-y-2">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="tabular-nums text-xs text-muted-foreground">
                  {slider.id === "blendFactor" ? slider.value.toFixed(2) : slider.value}
                </span>
              </div>
              <Slider
                value={[slider.value]}
                ...
                step={slider.step}
                onValueChange={([value]) =>
                  onChange({ [slider.id]: slider.id === "blendFactor" ? Number(value) : Math.round(value) } as Partial<MosaicControlsState>)
                }
              />
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="flex items-center justify-between rounded-xl border border-border/60 bg-muted/20 px-4 py-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Run Torch on CUDA when available</p>
            </div>
            <Switch checked={state.preferGpu} onCheckedChange={(checked) => onChange({ preferGpu: checked })}>
          ...
          <div className="flex items-center justify-between rounded-xl border border-border/60 bg-muted/20 px-4 py-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Enable Floyd–Steinberg on color fallback</p>
            </div>
            <Switch checked={state.applyDither} onCheckedChange={(checked) => onChange({ applyDither: checked })}>
